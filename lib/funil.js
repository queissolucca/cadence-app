...
export function proximoPasso({ pago, nome }) {
  /* O PAGAMENTO SAIU DAQUI.

     Ele era o primeiro passo obrigatório: quem criava conta e não pagava era
     mandado pro /pagamento e não via o produto. No freemium isso se inverte —
     a pessoa entra, escreve com a Cady, e só encontra o caixa quando tenta
     FALAR ou abrir a TRILHA, com um popup que diz o que ela ganha.

     O parâmetro `pago` continua na assinatura de propósito: quem chama passa
     o dado, e tirá-lo obrigaria a mexer no middleware por uma mudança que é
     de política, não de estrutura. Se o modelo voltar a ser pago-na-porta, a
     linha volta aqui e em nenhum outro lugar. */
