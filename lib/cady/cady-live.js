...
export const STATE_LAYERS = {
  ...
  /* rindo: a cara PADRÃO dela falando.

     Era `talking` (olho de bolinha, boca esticando), que é uma cara neutra —
     alguém emitindo som. A Cady que o produto vende está gostando da conversa,
     e isso mora nos OLHOS: `eye-arc` é o olho fechado de riso, e a boca aberta
     já tem língua (o `m-open` daqui é mOpen(...,tongue=true)).

     Não pisca, e está certo: olho de riso já é olho fechado, e piscar um traço
     curvo não lê como nada — é o mesmo motivo pelo qual `mudo` ficou de fora do
     PISCAVEL. A vida aqui vem da boca, que segue a amplitude real da voz, e da
     cabeça, que se mexe. */
  rindo: ["eye-arc", "m-open"],
...
/* DURANTE A CONVERSA, A COROA PARA DE CUSTAR.

   Cada um dos 72 raios roda uma animação CSS infinita de transform: scaleY.
   Animação de transform em SVG não é compositada em nenhum navegador: o motor
   recalcula estilo e refaz LAYOUT de cada raio a cada quadro, mesmo com o
   JavaScript parado. Medido: 142ms de thread principal por segundo só com o SVG
   montado e zero escrita de JS, com 86 animações rodando ao mesmo tempo.

   Numa conversa de voz isso disputa a thread principal com a captura do
   microfone, o playback e o WebSocket — e no iOS é a thread principal que
   responde aos avisos de pressão de memória do sistema. Saturada, ela não
   responde, e o WebKit mata a aba. É o "esta página web foi recarregada devido
   a um problema" que o usuário viu.

   O respiro da coroa é encanto de tela em repouso. Enquanto ela fala, quem
   carrega o "está viva" é a boca seguindo a voz, a cabeça se mexendo e as
   piscadas — tudo isso continua. A coroa fica parada, que é o mesmo estado que
   o modo "menos movimento" já usa.

   O gancho é o atributo no <html>, posto pela tela da conversa, e não o
   data-live do próprio SVG: data-live vale pra toda Cady animada por JS
   (prévia, styleguide), e congelar a coroa em todas elas seria cobrar de telas
   que não têm conversa nenhuma rodando. */
html[data-conversa="ativa"] .cady-ray{animation:none}
