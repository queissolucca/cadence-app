...
export const TEAR = "#DDF2FF", TONGUE = "#F2758F", ANGER = "#FF5A4E";
...
/* Mistura dois hex. Usada só pelo `heat`: na imagem 2 a coroa e o brilho
   esquentam junto com o corpo — deixá-los verdes num rosto vermelho é o que
   mais denuncia que a cara brava foi remendada por cima da normal. */
const hex = h => [1,3,5].map(i => parseInt(h.slice(i, i+2), 16));
export function mix(a, b, t) {
  const A = hex(a), B = hex(b);
  return "#" + A.map((v, i) => Math.round(v + (B[i]-v)*t).toString(16).padStart(2, "0")).join("");
}
export const RAY_QUENTE = "#FFA24E", GLOW_QUENTE = "#D93A2F";
...
<radialGradient id="heat${uid}" cx="0.5" cy="0.56" r="0.55">
 <stop offset="0" stop-color="#FF6A4A" stop-opacity="0.30"/>
 <stop offset="0.58" stop-color="#E23B33" stop-opacity="0.62"/>
 <stop offset="1" stop-color="#A31C22" stop-opacity="0.86"/></radialGradient>
...
// --------------------------------------------------------- cara de brava
/* Sobrancelha inclinada: a ponta de DENTRO desce. É a inclinação, e não a
   espessura, que faz o rosto ler como bravo — invertida, a mesma linha vira
   surpresa. */
export function brows(P, k = 1) {
  const w = ARC_W*R*1.02, th = ARC_TH*R*0.95;
  return [-1, 1].map(s => {
    const [x, y] = eyePos(s), yy = y - E_R*R*1.75;
    const ox = x + s*w/2, oy = yy - w*0.16*k;   // fora, mais alto
    const ix = x - s*w/2, iy = yy + w*0.22*k;   // dentro, mais baixo
    return st(`M ${f1(ox)} ${f1(oy)} L ${f1(ix)} ${f1(iy)}`, P.ink, th);
  }).join("");
}

/* Dentes cerrados: retângulo escuro com as barras claras por cima. As barras
   são desenhadas com cap "butt" pra encostarem na borda sem estourar. */
export function mGrit(P) {
  const w = M_RX*R*5.0, h = M_RY*R*1.9, y = CY + M_DY*R, x0 = CX - w/2;
  let out = `<rect x="${f1(x0)}" y="${f1(y-h/2)}" width="${f1(w)}" height="${f1(h)}" ` +
    `rx="${f1(h*0.20)}" fill="${P.ink}"/>`;
  for (let i = 1; i < 7; i++) {
    const x = x0 + w*i/7;
    out += st(`M ${f1(x)} ${f1(y-h/2)} L ${f1(x)} ${f1(y+h/2)}`, "#FFFFFF", h*0.12, 0.9, "butt");
  }
  out += st(`M ${f1(x0)} ${f1(y)} L ${f1(x0+w)} ${f1(y)}`, "#FFFFFF", h*0.12, 0.9, "butt");
  return out;
}

/* Vapor saindo da cabeça. */
export function exSteam(P) {
  const cfg = [[-0.50,-1.26,0.17],[0.08,-1.40,0.20],[0.60,-1.24,0.16]];
  return cfg.map(([dx,dy,sc],i) => {
    const x = CX+dx*R, y = CY+dy*R, w = sc*R;
    return `<g class="cady-steam" style="--i:${(i*0.28).toFixed(2)}s">` +
      st(`M ${f1(x)} ${f1(y+w)} C ${f1(x-w*0.58)} ${f1(y+w*0.45)} ` +
         `${f1(x+w*0.58)} ${f1(y-w*0.05)} ${f1(x)} ${f1(y-w*0.60)}`,
         "#CFC6CB", w*0.17, 0.60 - 0.09*i) + `</g>`;
  }).join("");
}

/* As marcas ")(" de raiva, uma de cada lado da cabeça. */
export function exAnger(P) {
  return [[-1.10,-0.80,-1],[1.10,-0.84,1]].map(([dx,dy,dir]) => {
    const x = CX+dx*R, y = CY+dy*R, w = 0.14*R;
    return st(`M ${f1(x-dir*w*0.55)} ${f1(y-w)} Q ${f1(x+dir*w*0.5)} ${f1(y)} ` +
              `${f1(x-dir*w*0.55)} ${f1(y+w)}`, ANGER, w*0.28, 0.92);
  }).join("");
}
...
export const EXPRESSIONS = {
  ...
  // --- as bravas (imagem 2). `heat` é o quanto ela avermelha, 0..1 ---
  "incomodada":  { eyes: both(eDot),   mouth: mWave,  extra: exSweat, blush: 0.58, heat: 0.15 },
  "seria":       { eyes: P => brows(P) + both(eDash)(P), mouth: mLine, blush: 0.50, heat: 0.20 },
  "brava":       { eyes: P => brows(P) + both(eDot)(P),  mouth: mWave, blush: 0.70, heat: 0.35 },
  "irritada":    { eyes: P => brows(P) + ePupil(P,-1,[0,0]) + ePupil(P,1,[0,0]),
                   mouth: mFrown, blush: 0.55, heat: 0.45 },
  "cerrando":    { eyes: P => brows(P) + both(eDot)(P),  mouth: mGrit, blush: 0.66, heat: 0.60 },
  "fumegando":   { eyes: P => brows(P) + both(eDot)(P),  mouth: mGrit,
                   extra: exSteam, blush: 0.70, heat: 0.75 },
  "explodindo":  { eyes: P => brows(P) + ePupil(P,-1,[0,0]) + ePupil(P,1,[0,0]),
                   mouth: mGrit, extra: P => exSteam(P) + exAnger(P), blush: 0.80, heat: 1 },
  "provocando":  { eyes: P => brows(P) + both(eDot)(P),
                   mouth: (P,u) => mOpen(P,u,true,1.0),
                   extra: P => exSteam(P) + exSparks(P), blush: 0.75, heat: 0.55 },
  "desconfiada": { eyes: P => brows(P) + both(eArc)(P),  mouth: mSmirk, blush: 0.60, heat: 0.10 },
};
...
export function cady(opts = {}) {
  ...
  const P0 = typeof palette === "string" ? PALETTES[palette] : palette;
  const E = EXPRESSIONS[expression];
  const P = E.heat
    ? { ...P0, ray: mix(P0.ray, RAY_QUENTE, E.heat), glow: mix(P0.glow, GLOW_QUENTE, E.heat) }
    : P0;
  let s = defs(P, uid);
  ...
  s += `<g class="core">` + volume(P, uid) +
       // As caras bravas avermelham por cima do corpo, com a intensidade que a
       // própria expressão declara — não por troca de paleta.
       (E.heat ? `<circle cx="${CX}" cy="${CY}" r="${R}" fill="url(#heat${uid})" ` +
         `opacity="${E.heat.toFixed(2)}"/>` : "") +
       blush(P, E.blush) +
