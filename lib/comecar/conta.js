...
/* O SDK DO SUPABASE ENTRA SÓ QUANDO ALGUÉM VAI USAR CONTA.
   ...
   todo mundo que abria o site baixava e executava o SDK de
...
export async function criarConta({ nome, sobrenome, email, senha, convite }) {
  ...
  const { data, error } = await supabase.auth.signUp({
    ...
    password: senha,
    options: {
      /* Direto pro app, não pro caixa. Isto apontava pro /pagamento, de quando
         o produto inteiro era pago; com o freemium, escrever é grátis, e mandar
         quem acabou de confirmar o e-mail pra uma tela de preço é pedir a
         compra antes de entregar o motivo. Mesmo destino do cadastro com sessão
         (ver `seguir`), pra os dois caminhos terminarem no mesmo lugar. */
  ...
  /* CONTA QUE JA EXISTE NAO DA ERRO — DA UM SUCESSO FALSO.

     Pra nao revelar quais e-mails tem cadastro, o Supabase responde a um
     signUp de e-mail ja registrado exatamente como responderia a um novo:
     200, sem erro, `session` null. A diferenca esta num lugar so —
     `identities` volta como lista VAZIA. E ele nao manda e-mail nenhum.

     Sem olhar isso, quem ja tinha conta via "Confirma seu e-mail" e ficava
     esperando pra sempre um link que nunca foi enviado. */
  const jaExiste = Array.isArray(data?.user?.identities) && data.user.identities.length === 0;
  return { session: data.session, jaExiste };
}

/* Reenvia a confirmacao pra quem criou a conta e nao recebeu (ou perdeu) o
   link. O Supabase responde 200 mesmo quando nao manda nada, entao isto nunca
   promete entrega — so dispara e devolve. */
