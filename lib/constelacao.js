...
const PULL = 43;       // deslocamento máximo, px (ímã a 90%, validado)
...
/* A MESMA MALHA NO CLARO.

   Verde da marca sobre branco quase some: #3E9B5F a 34% de opacidade some
   no papel do jeito que brilhava no preto. Então a linha desce pro verde
   escuro, o ponto fica no verde da marca (ele é cheio, aguenta), e o mint
   #8FF0C0 — que no claro viraria um borrão pálido — vira um verde vivo.

   `alpha` é 1.155 porque o valor validado foi "força 66%" no estudo, e lá a
   escala era 1.75× a daqui: 0.66 × 1.75. Está acima de 1 de propósito — ele
   multiplica opacidades que nascem em 0.34, então o teto real não chega perto
   de saturar. */
export const CORES_CLARAS = { linha: '32,92,57', ponto: '62,155,95', toque: '14,156,103', alpha: 1.155 };
...
export function montarConstelacao(cv, { cores = () => CORES_ESCURAS, alvoPonteiro } = {}) {
  ...
  function init() {
    ...
    /* Densidade validada em 155% do que era (o estudo de tema claro). O divisor
       caiu de 4600 pra 2968 (= 4600 / 1.55) e o teto subiu junto — mantê-lo em
       110 faria a densidade nova não aparecer em tela grande, que é justamente
       onde ela foi aprovada. O teto continua existindo porque o custo dos pares
       cresce mais rápido que o número de pontos. */
    const n = Math.max(30, Math.min(170, Math.round(w * h / 2968)));
  ...
  function frame() {
    ...
    for (const p of pts) {
      ...
      let tx = 0, ty = 0;
      if (ptr.on && !parado) {
        /* dx aponta DO PONTO PARA O CURSOR, e é somado: o ponto caminha na
           direção do ponteiro. É ímã, não repulsão — inverter qualquer um dos
           dois sinais aqui faz a malha passar a fugir do dedo. */
        const dx = ptr.x - p.hx, dy = ptr.y - p.hy, d2 = dx * dx + dy * dy;
        if (d2 < PULL_R2 && d2 > 0.25) {
          const d = Math.sqrt(d2);
          const k = (1 - d / PULL_R) * (ptr.down ? 1.8 : 1);
          /* Trava de ultrapassagem: sem ela, um ponto a 10px do cursor recebe
             deslocamento de PULL px e atravessa pro outro lado, ficando a
             tremer em volta do ponteiro em vez de encostar nele. */
          const desloc = Math.min(PULL * k, d * 0.8);
          tx = dx / d * desloc;
          ty = dy / d * desloc;
        }
      }
