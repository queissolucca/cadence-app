...
- "rubrica_delta": para precisao/naturalidade/vocabulario/fluencia, -1 se a resposta piorou nesse eixo, 0 se neutro/não evidenciado, +1 se demonstrou domínio. A maioria fica 0 — só marque quando a resposta realmente evidenciar aquele eixo especificamente.`;
  ...
  try {
    const response = await client.messages.create({
      ...
      // Sonnet 5 roda "adaptive thinking" por padrão quando o parâmetro é
      // omitido — desativamos porque é uma extração/pontuação direta, sem
      // necessidade de raciocínio extra (latência/custo). Haiku 4.5 não tem
      // essa troca de comportamento, então não mexemos no path rápido.
      ...(isQuick ? { temperature: 0 } : { thinking: { type: 'disabled' } }),
      output_config: {
