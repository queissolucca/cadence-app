import { dayKeySP, prevDayKey } from './dates';

// Streak a partir do HISTÓRICO de sessões (fonte da verdade) — dias consecutivos
// com atividade, terminando hoje ou ontem. Robusto e auto-corrigível: sempre
// bate com o calendário, sem depender do contador incremental streak_count (que
// dessincroniza se um update falhar). Se o dia mais recente com sessão não for
// hoje nem ontem, a sequência quebrou -> 0.
export function streakFromDayKeys(doneSet, todayKey = dayKeySP(new Date())) {
  let cursor = todayKey;
  if (!doneSet.has(cursor)) {
    cursor = prevDayKey(todayKey); // ainda pode estar vivo desde ontem
