...
import { identidadeDe } from './supabase/middleware';
...
export const identidade = cache(async () => {
  ...
  // Blindado do mesmo jeito que no middleware, e pelo mesmo motivo: o getClaims
  // RELANÇA o que não for erro de auth (um fetch do JWKS que falhou, por
  // exemplo), e aqui isso viraria "Application error" na tela em vez de uma
  // página lenta. Ver identidadeDe em lib/supabase/middleware.js.
  return identidadeDe(supabase);
});
