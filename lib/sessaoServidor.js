...
export const temPlanoCompleto = cache(async () => {
  ...
  const supabase = createClient();
  let r = await supabase.from('paid_emails').select('email, expires_at').eq('email', eu.email).maybeSingle();
  if (r.error) r = await supabase.from('paid_emails').select('email').eq('email', eu.email).maybeSingle();
