...
/* Esta pessoa tem o plano completo?

   O middleware já responde isso pra barrar a API, mas o SHELL também precisa —
   pra pôr o cadeado na aba da trilha e decidir se o toque abre a tela ou o
   popup. Ler duas vezes por requisição seria desperdício, e por isso passa
   pelo mesmo `cache()` do resto daqui.

   Na dúvida (a consulta falhou) devolve FALSE, e não true: errar pra menos
   mostra um cadeado a quem pagou — irritante, e a pessoa recarrega. Errar pra
   mais abre a voz de graça, e cada minuto de ElevenLabs custa. O portão duro
   continua sendo o da API, que também nega na dúvida. */
export const temPlanoCompleto = cache(async () => {
  const eu = await identidade();
  if (!eu?.email) return false;
  const supabase = createClient();
  /* Sem o fallback que buscava só o email: ele tratava linha sem data como
     acesso permanente, e um erro transitório viraria acesso a quem expirou.
     Ver o mesmo trecho em middleware.js. */
  const r = await supabase
    .from('paid_emails').select('email, expires_at').eq('email', eu.email).maybeSingle();
  if (r.error) return false;
  const linha = r.data;
  // expires_at nulo = acesso sem expiração (contas antigas).
  return !!linha && (!linha.expires_at || new Date(linha.expires_at) > new Date());
});
