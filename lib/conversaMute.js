...
export function decidirMute({ ativo, falando, mudo, nosso, assumido }) {
  /* Sessão fechada — e aqui "fechada" inclui um caso que não parece fechamento:
     o SDK do React põe `status` em 'error' em QUALQUER onError, inclusive nos
     que não derrubam a conexão. Quando isso acontece no meio da fala da Cady, o
     microfone está fechado POR NOSSA CONTA, e a versão anterior devolvia
     `mudar: null` e ainda apagava `nosso` — ou seja, esquecia que tinha uma
     dívida e deixava o microfone fechado pra sempre. A pessoa fala, repete, e
     não é ouvida; o que ela vê é a Cady tendo parado de responder.

     A memória some (é o certo: acabou a sessão), mas antes de sumir a gente
     paga o que devia. Devolver `false` com a sessão já encerrada não custa
     nada — o setMuted vai num try/catch. E só devolvemos quando há o que
     devolver: microfone já aberto não precisa ser reaberto. */
  if (!ativo) return { mudar: nosso && mudo ? false : null, nosso: false, assumido: false };
