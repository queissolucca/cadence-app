...
export async function completeSession(supabase, user, { kind, mode, duration_seconds, items_total, items_correct }) {
  ...
  // Recorde de streak (best-effort: a coluna streak_max pode não existir ainda —
  // migration 0013). Se não existir, o select falha e a gente só ignora.
  const { data: maxRow } = await supabase.from('profiles').select('streak_max').eq('id', user.id).maybeSingle();
  if (maxRow && typeof maxRow.streak_max === 'number' && streakResult.streakCount > maxRow.streak_max) {
    await supabase.from('profiles').update({ streak_max: streakResult.streakCount }).eq('id', user.id);
  }
