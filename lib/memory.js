...
/* Teto de fatos por usuário. Era 40, e o número não é decoração: este bloco
   inteiro é reenviado ao agente de voz a CADA TURNO da conversa, então cada fato
   guardado é um custo recorrente, não um custo de escrita.

   Medido: 40 fatos davam ~3.200 caracteres, uns 890 tokens por turno. Em 20 cai
   pra ~445. O que se perde é memória longa — ela lembra das 20 coisas mais
   marcantes em vez das 40, e as mais antigas e menos importantes caem primeiro
   (o corte é por importância, depois por data). */
export const MEMORY_CAP = 20;

// A lista em si vive em ./memoryCategories, que não importa nada de servidor —
// assim as telas de cliente usam os mesmos rótulos sem puxar o SDK da Anthropic
// pro bundle. Re-exportado aqui pra não quebrar quem já importava daqui.
import { CATEGORIES, CATEGORY_LABELS } from './memoryCategories';

// re-export puro (`export ... from`) não traz os nomes pro escopo deste
// módulo, e o schema de extração aqui embaixo usa CATEGORIES — por isso
// importa e reexporta em vez de só reexportar.
