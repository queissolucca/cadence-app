...
/* Até onde vai o acesso, contado em DIAS.

   Era `threeMonthsFrom()`, com os três meses no nome da função. Isso bastava
   enquanto havia um plano só; com o semanal, qualquer chamada esquecida daria
   90 dias por R$ 19,90 — e o erro não apareceria em teste nenhum, só na
   fatura. Agora a duração vem de quem chama, e quem chama lê de plans.js.

   Dias e não meses porque "uma semana" não tem tradução em mês, e misturar as
   duas unidades no mesmo cálculo é como aparecem os erros de um dia. */
export function validadeEmDias(dias, date = new Date()) {
...
export function classifyEvent(event) {
  ...
  /* REVOGACAO VEM PRIMEIRO, E DE PROPOSITO.

     Um evento de reembolso chega com o payload do pagamento ORIGINAL, que
     ainda diz status PAID. Se ele nao for reconhecido como revogacao aqui, o
     teste de "parece pago" mais adiante ve esse PAID e LIBERA 3 meses de
     acesso pra quem acabou de ser reembolsado.

     A doc do AbacatePay tem DUAS familias de cobranca — `checkout.*` e
     `transparent.*` — e cada uma tem refunded/disputed/lost. A versao
     anterior so cobria checkout.refunded e checkout.disputed: os outros
     quatro eventos caiam no grant. */
  if (/^(checkout|transparent)\.(refunded|disputed|lost)$/.test(e)) return 'revoke';
  if (/(billing\.refunded|payment\.refunded)/.test(e)) return 'revoke';
  if (/((checkout|transparent)\.completed|subscription\.completed|subscription\.renewed|billing\.(recurring\.)?paid|pix\.paid|payment\.(approved|confirmed|paid|succeeded))/.test(e)) return 'grant';
