...
export default async function HojePageV2() {
  ...
  const { data: stateRows } = await supabase
    ...
    .select('scenario_id, mastered_count, status')
    .eq('user_id', user.id);
  ...
  try {
    ...
    content = { phrase_of_day: null, exercises: { writing: [], speaking: [] } };
  }
  ...
  const { data: dueForBadges } = await supabase
    .from('review_items')
    .select('id, skill')
    .eq('user_id', user.id)
    .eq('mastered', false)
  ...
  const { data: weekSessions } = await supabase
    ...
    .select('started_at')
    .eq('user_id', user.id)
    .gte('started_at', weekStartSP(now).toISOString())
  ...
  // Frases due hoje (máx 5)
  const { data: duePhrases } = await supabase
    .from('review_items')
    .select('id, pattern, content, next_review_at')
    .eq('user_id', user.id)
    .eq('mastered', false)
    .lte('next_review_at', nowIso)
  ...
  return (
    <>
      ...
      {/* 3 */}
      <h1 style={{ fontSize: 31, fontWeight: 800, letterSpacing: '-1px', margin: 0, color: 'var(--ink)' }}>O que vamos praticar hoje?</h1>

      {/* 4. Card Writing */}
      <Link href="/v2/praticar/writing" style={{ textDecoration: 'none' }}>
        <div className="v2-card-dark" style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
      ...
      <Link href="/v2/praticar/speaking" style={{ textDecoration: 'none' }}>
        <div className="v2-card-green" style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
